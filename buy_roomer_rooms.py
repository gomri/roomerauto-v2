...
def buy_roomer_amazing_deal(driver):
	...
	print_test_report(TEST_CASE_PARAMS['source_roomer'],
						TEST_CASE_PARAMS['test_cases']['roomer_ad_open_sd_buy_random'],
						cancellation_policy,
						supplier_type,
						rate_plan,
						tested)
	...
	insert_data_to_dict(TEST_CASE_PARAMS['source_roomer'],
						...
						rate_plan,
						tested,
						TEST_TO_RUN['roomer_ad_open_sd_buy_random'])
...
def buy_roomer_top_100(driver):
	...
	driver.switch_to.window(driver.window_handles[-1])
	try:
		cancellation_policy, supplier_type, rate_plan = click_LH(driver)
	except NoSuchElementException:
		cancellation_policy, supplier_type, rate_plan = click_FC(driver)
	sleep(5)
	fill_in_review_page(driver)
